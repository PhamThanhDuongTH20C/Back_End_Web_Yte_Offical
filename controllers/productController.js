...
const getnameProduct = async (req, res) => {
  try {
    ...
    const normalizedPartialProductName = productName
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/[^\w\s()]/g, ""); 
    if (productName.trim() === '') {
    ...
    const products = await Product.find({
      $or: [
        {
          productName: {
            $regex: new RegExp(escapeRegex(normalizedPartialProductName.trim()), "i"),
          },
        }, 
    ...
    const productsData = await Promise.all(
      products.map(async (product) => {
        const [specifications, imageProducts, brand, category] = await Promise.all([
...
const searchProductWithPagination = asyncHandler(async (req, res) => {
  const { keyWord, priceRange, page = 1, pageSize = 20, categoryId } = req.body;
  try {
...
const recommendationsProduct = asyncHandler(async (req, res) => {
  try {
    const allProducts = await Product.find(
      {},
      { sku: 1, productName: 1, sellingPrice: 1, urlImage: 1 }
    );
